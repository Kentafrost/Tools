...
# Check if the folder exists, then delete it(if nesessary)
...
def google_authorize():
    ...
    try:
        ...
        credentials = Credentials.from_service_account_file("XXXXXXX", scopes=scope)
...
def listup_all_files(folder_path, sheet):
    ...
    for folder_name in folder_names: # each path in folder_path
        ...
        #print(f"子フォルダーパス(キャラ名): {file_path}")
        
        # check if file exists in each folder
        try:
            if not len(os.listdir(file_path)) == 0:
                files = os.listdir(file_path) # get all file
